...
# the filepath of the unprocessed dataset, should be a copy of the original,
# DO NOT use the original files/dataset.
data_loc = './tech_dataset_y/'
processed_dataset_loc = './processed_dataset_2/'


items = os.listdir(data_loc)                    # create list of filenames which are in the input directory
folders = clean(items)
        # print(items)
        # print(folders)
...
for folder in folders:
    # if the /csvs and /csvs/processed_csvs not in directory, create them.

    os.mkdir(data_loc + folder + '/csvs')

    os.mkdir(data_loc + folder + '/csvs/processed_csvs')

    files = os.listdir(data_loc + folder)             #list
    files_processed = filter('.mid', files)
    for f in files_processed:
        f.replace(' ', '\\ ')

    pathtomidi = data_loc + folder + '/'
    pathforcsv = data_loc + folder + '/csvs/'
...
# CSV processing
for folder in folders:
    unprocessed_csvs = os.listdir(data_loc + folder + '/csvs/')
    filtered_csvs = filter('.csv', unprocessed_csvs)

    for csv in filtered_csvs:                           # for each csv file in /csvs/
        fp = open(data_loc + folder + '/csvs/' + csv)           # open csv as file object
        fp_ls = []
        ...
        fp_processed = open(data_loc + folder + '/csvs/processed_csvs/' + csv, 'w')
        for line in fp_ls:
...
# re-encode as midi
for folder in folders:
    csv_files = os.listdir(data_loc + folder + '/csvs/processed_csvs')
    csv_files_processed = filter('.csv', csv_files)

    pathtocsv = data_loc + folder + '/csvs/processed_csvs/'
    pathformidi = processed_dataset_loc
